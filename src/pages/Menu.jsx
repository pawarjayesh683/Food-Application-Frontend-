...
const Menu = () => {
    ...
    if(error) return <p className='text-blue-500'>Loading Menu...</p>
    if(error) return <p className='text-red-500'>{error}</p>
    
  return (
    <div className='max-w-7xl mx-auto'>
        <h2 className='text-center font-bold text-3xl my-3'>Menu's</h2>
        {
            menuItems.length === 0 && (
                <p>No items Availabe</p>
            )
        }
            ...
            {menuItems.map((item)=> (
                <div key={item._id} className='border p-2 rounded-md shadow-xl shadow-blue-800'>
                    ...
                    <div className='space-x-4 p-2'>
                        <button className='border px-5 text-xl rounded-sm' onClick={() => updateQuantity(item._id,-1)}> - </button>
